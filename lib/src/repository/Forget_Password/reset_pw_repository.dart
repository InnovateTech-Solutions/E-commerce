...
import 'package:ecommerce/src/getx/reset_pw_controller.dart';
...
class ResetPasswordRepository extends GetxController {
  ...
  Future<void> updatePasswordFirestore() async {
     _db.collection("User").
    where('Email', isEqualTo: userController.userModel.email).get().then((querySnapshot) {
      if (querySnapshot.docs.isNotEmpty) {
       var userDoc = querySnapshot.docs.first;
        userDoc.reference.update({'Password': resetPwController.newPassword.text.trim()});
    }
  });
  } 
    ...
    try {
      ...
      if (user != null) {
        ...
        if (newPassword != confirmPassword) {
          Get.snackbar("Failure", "New password and confirm password do not match",
          ...
          backgroundColor: ColorConstants.snakbarColorError);
              ...
              // Update the password
        await user.updatePassword(newPassword).whenComplete(() { Get.snackbar(
